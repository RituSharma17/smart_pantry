...
import json
import io
import sql_openai_config as sql_openai_config
...
# ── lazy-load EasyOCR so startup is fast even if unused ──
_ocr_reader = None
def get_ocr_reader():
    global _ocr_reader
...
def _validate_and_normalize_expiry(expiry_date: str | None):
...
# ── FIX 1: use LIKE for partial name matching so brand-prefixed names still match ──
def remove_pantry_item(name):
    conn = get_connection()
    cur  = conn.cursor()
    # ── removed LIMIT 1 so ALL matching rows (duplicates) are deleted at once ──
    cur.execute(
...
# ── FIX 2: added rule 9 so agent retries with exact name on "not found" ──
SYSTEM_PROMPT = """
...
8) If no expiry date is visible, estimate one based on the product type using these
   shelf-life guidelines (from today's date):
...
2. For each item, call add_pantry_item to add it to the pantry.
...
4. For expiry dates:
   ...
   - If NO expiry date is visible, estimate one from the ocr text provided above as start date using these guidelines:
       Fresh produce (leafy greens, berries)    → +5 days
       Fresh produce (root veg, citrus, apples) → +14 days
       Fresh meat & poultry                     → +3 days
       Fresh seafood                            → +2 days
...
6. After adding all items, reply with a summary like:
   ...
   Always append "(estimated)" when the date was not found on the label,
   and "(from label)" when it was explicitly visible in the OCR text.
   This text will be displayed directly to the user so it must always be present.
7. If no food items are found, say so clearly.
"""
...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Remove an ingredient from the pantry by name (partial match supported).",
            "parameters": {
                "type": "object",
...
# ── FIX 3: guard against msg.content being None ──
def run_agent_turn(messages) -> str:
    ...
    while True:
        ...
        if not getattr(msg, "tool_calls", None):
            return msg.content or "Done."  # ← guard against None
        ...
        for tc in msg.tool_calls:
            ...
            result = TOOL_MAP[fn_name](fn_args) if fn_name in TOOL_MAP else f"ERROR: unknown tool '{fn_name}'"
            messages.append({
                "role": "tool",
                "tool_call_id": tc.id,
...
def ocr_upload():
    ...
    try:
        ...
        conversation = [
            ...
            {"role": "user",   "content": OCR_PARSE_PROMPT.replace("{today}", today_str).replace("{ocr_text}", raw_text)},
        ]
        reply = run_agent_turn(conversation)
